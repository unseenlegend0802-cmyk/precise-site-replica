  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      const { error } = await supabase.auth.signInWithOAuth("google", {
        redirectTo: `${window.location.origin}${redirectAfterAuth}`,
      });
  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-xs text-muted-foreground text-center">
        We use Gmail OAuth for a secure, passwordless login experience. Your data is protected with industry-standard encryption.
      </p>
    </div>
  );
};
